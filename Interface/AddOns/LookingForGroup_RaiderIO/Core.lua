...
local function generate_raider_io_info(unit)
	...
	if info then
		...
		for k,v in pairs(dungeons) do
			restb[#restb+1] = {C_LFGList_GetActivityGroupInfo(k),info_dungeons[v]}
		end
				...
				for bagID = 0, 4 do
					for invID = 1, GetContainerNumSlots(bagID) do
						...
						if itemID and itemID == 138019 then
							...
							if activity_id then
								LookingForGroup_Options.listing(activity_id,
										{title="+"..keyLevel,minimum_item_level = GetAverageItemLevel()-10})
