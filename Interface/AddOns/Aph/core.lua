...
function APH:OnInitialize()
	...
	APH:RegisterEvent("ZONE_CHANGED",APH:ZONE_CHANGED())
	APH:RegisterEvent("ZONE_CHANGED_INDOORS",APH:ZONE_CHANGED())
	APH:RegisterEvent("ZONE_CHANGED_NEW_AREA",APH:ZONE_CHANGED())
...
function APH:bankToggle(val)
	if val then
		...
		APH:UnregisterEvent("BANKFRAME_CLOSED",APH:BANKFRAME_CLOSED())
		APH:UnregisterEvent("BANKFRAME_OPENED",APH:BANKFRAME_OPENED())
	end
        ...
        for slot = 1, GetContainerNumSlots(bag) do
			...
			if a==L["Empowering"] or a== "Potencializando" or a=="Fortalecimento" or a=="Potencializador" then
				...
				if bank then
					...
					conf=APH:TableContains(APTable,id,1)
					--APH:Print(conf)
					if conf then
...
function APH:ReadAP (itemLink,pp)
	...
	for i=tip:NumLines(),1,-1 do
		local txt=_G["APHTooltipReaderTextLeft"..i]:GetText()
	  if txt and string.match(string.gsub(string.gsub(txt,"%p",""),"%d+([ ])?%d+",""),L["Use Grants (%d+)"]) then
		if string.match(txt,L["(%d+) Artifact"]) then -- Under 1 million
			...
			value,power = string.match(txt,"(%d*[,%.]?%d+)%s(%a)")
			local conv={['m']=1e6,['b']=1e9,['t']=1e12}
			--APH:Print(value, power)
			ap=value * (conv[power] or 1)
		end
		
		return tonumber(ap)-->10 and tonumber(power) or tonumber(power)*1000000
	  end
	end	
	return 0
end
...
APHMover = CreateFrame("Frame","APHMover",UIParent)
	...
	APHMover.Mini:SetPoint("CENTER",APHMover)
